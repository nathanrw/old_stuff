...
class BillScythe(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.anims.append(ImportPNG("billscytheburn.png"))
        self.currentimg = self.anims[0]
        self.image = self.anims[0]
        self.rect = self.image.get_rect()

        self.armjoint = (36,200)
        self.rect.left = self.armjoint[0] - 5
        self.rect.top = self.armjoint[1] - 81
    ...
    def _rotate(self,angle):
        ...
        self.image = pygame.transform.rotate(self.currentimg,angle)
        self.rect = self.image.get_rect()
...
def main():
    ...
    Black = []
    Black.append(ImportPNG("black_1.png"))

    White = []
    White.append(ImportPNG("white.png"))
    ...
    billscythe = BillScythe()

    #Sprite groups
    global chilve_sprite
    chilve_sprite = pygame.sprite.RenderClear([billscythe,bill,billgun])
    global laser_sprite
    ...
    while 1:
        ...
        if billdead == 0:
            ...
            isswiping = pygame.mouse.get_pressed()[2]
            #Shoots for Bill             
            if isshooting:
                if shotlim == 4:
                ...
                for dude in flan_sprites:
                    if sprite.rect.center[1] > dude.hitbox[2]  \
                       ...
                       and dude.state != "dead":
                        ...
                        if dude.name == "rasputin" and dude.state == "rasputin_idle":
                            sprite.xvel = -sprite.xvel
                            sprite.yvel = -sprite.yvel
                            sprite.anim_fly[0] = pygame.transform.rotate(sprite.image,180)
                        else:
                            #Explosions
                            sprite.kill()
            ...
            for sprite in flan_sprites:
                if sprite.rect.center[0] < (bill.rect.right+200) \
                   and sprite.rect.center[0] > (bill.rect.left-30) \
                   and sprite.rect.center[1] > bill.rect.top-10 \
                   and sprite.rect.center[1] < bill.rect.bottom+10:
                    
                    if billscythe.state == "swiping":
                        if sprite.state != "dead" and sprite.state != "rasputin_idle":
                            sprite.destroy()
                            killcounter += 1
                    ...
                    for sprite in laser_sprite: #left right top bottom
                        if sprite.rect.center[0] > ball.hitbox[0]  \
                           ...
                           and sprite.rect.center[1] < ball.hitbox[3]:
                            ball.xvel += sprite.xvel/50.0
                            ball.yvel += sprite.yvel/50.0
                            sprite.kill()
                    if ball.rect.left < bill.rect.right+200 and ball.rect.left > bill.rect.left+30 and billscythe.state == "swiping":
                        ball.xvel += 0.8
                    for dude in flan_sprites:
        ...
        if billdead == 0:
            ...
            billscythe._move((bill.rect.left+34,bill.rect.top+38))
